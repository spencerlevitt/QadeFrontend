...
export function getRecentGames(csrfToken, page, type) {
    return function(dispatch) {
        ...
        return getRecentGamesApi(csrfToken, page).then( data=> {
            if(type) {
                return dispatch(refreshRecentGamers(data.data)) && console.log(data, ">>>>><<<<<<<<")
            }
            return dispatch(recentGamers(data.data)) && console.log(data, ">>>>><<<<<<<<")
        })
        .catch(error => dispatch(recentGamersError(error)) && console.log(error, ">>>>><<<<<<<<"))
    }
}